...
# [关键修改 1] 输出目录
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'ablation_no_sentiment-H')
...
def get_combined_embedding(tweet_id, emotion_score, vector_dict):
    """
    [关键修改 2] 消融情感：只返回 BERT 向量，不拼接情感分数
    """
...
def main_gpu(args):
    ...
    if len(vector_dict) > 0:
        # [No Sentiment] 只有 BERT 维度 384
        sample_dim = next(iter(vector_dict.values())).shape[0]
    else:
        sample_dim = 384
    print(f"[Info] 向量维度 (无情感消融版): {sample_dim}")
    ...
    for w, w_t in tqdm(groups, desc="Progress", unit="win"):
        ...
        if num_edges > 0:
            tqdm.write(f"[{w.strftime('%Y-%m-%d')}] N:{G.number_of_nodes()} E:{num_edges} Mod:{mod_val:.3f} DCPRR:{dcprr_val:.3f} CNLR:{cnlr_val:.4f}")
        else:
    ...
    print("\n" + "="*60)
    print("               ABLATION (NO SENTIMENT) REPORT")
    print("="*60)
