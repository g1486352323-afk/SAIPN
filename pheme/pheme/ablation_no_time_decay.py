...
# [关键修改 1] 输出目录改为 ablation_no_time_decay
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'ablation_no_time_decay-H')
...
def calculate_vol_update(parent_sims, parent_times, parent_emotions, current_time_ts, current_emotion, decay_unit):
    """
    [关键修改 2] 消融时间衰减：观点更新时，不再考虑时间距离，只考虑相似度。
    """
    if len(parent_sims) == 0:
        return current_emotion
    
    # === ABLATION: NO TIME DECAY ===
    # 时间权重设为 1.0 (无衰减)
    contributions = parent_sims * parent_emotions
...
def main_gpu(args):
    ...
    for w, w_t in tqdm(groups, desc="Progress", unit="win"):
        ...
        for i, (idx, row) in enumerate(w_t.iterrows()):
            ...
            if current_pool_size > 0:
                ...
                if valid_indices.numel() > 0:
                    ...
                    v_times = full_times[:current_pool_size][valid_indices]
                    v_emos = full_emotions[:current_pool_size][valid_indices]
        ...
        pr_val, mod_val, assort_val, dcprr_val, cnlr_val = 0, 0, 0, 0, 0 # [新增] 初始化 cnlr_val
    ...
    print("\n" + "="*60)
    print("               ABLATION (NO TIME DECAY) REPORT")
    print("="*60)
