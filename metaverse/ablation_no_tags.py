...
# [Output] Tag 消融专用目录
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'ablation_no_tags-D')

EMBEDDING_DIR = os.path.join(BASE_DIR, 'embedding')
# [Input] 读取无 Tag 的向量文件
NEW_VECTOR_PATH = os.path.join(EMBEDDING_DIR, 'output_vectors_no_tags.txt')
...
def load_vectors_to_gpu(file_path):
    if not os.path.exists(file_path): 
        print(f"[Error] Vector file not found: {file_path}")
        return {}
    vector_dict = {}
...
def main_gpu(args):
    ...
    if not vector_dict:
        print("[Error] No vectors loaded. Exiting.")
        return
    ...
    # 获取维度 (No Tag BERT + Sentiment)
    base_dim = list(vector_dict.values())[0].shape[0]
    DIM = base_dim + 1 
    print(f"[Ablation Info] Vector Dimension: {DIM} (No Tag BERT + Sentiment)")
    ...
    # --- D. 循环 ---
    for w, w_t in tqdm(groups, desc="Simulating (No Tags)"):
    ...
    if all_stats_records:
        ...
        print("\n[Success] Ablation (No Tags) completed. Weighted averages saved.")
    else:
