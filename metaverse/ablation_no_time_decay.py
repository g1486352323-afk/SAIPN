...
# [Output] 时间衰减消融专用目录
OUTPUT_DIR = os.path.join(BASE_DIR, 'outputs', 'ablation_no_time_decay-D')
...
def main_gpu(args):
    ...
    # [Normal] BERT + Sentiment
    DIM = base_dim + 1 
    print(f"[Ablation Info] Vector Dimension: {DIM} (BERT + Sentiment)")
    ...
    # --- D. 循环 ---
    for w, w_t in tqdm(groups, desc="Simulating (No Time Decay)"):
        ...
        if history_count > 0:
            ...
            scores_h = sim_h * valid_time_mask # 仅应用硬截断，无软衰减
        ...
        # [关键修改] 同样移除 intra-batch 的时间衰减 (虽然通常很短)
        ...
        scores_in = sim_in * mask_in # 无 decay_in
    ...
    if all_stats_records:
        ...
        print("\n[Success] Ablation (No Time Decay) completed. Weighted averages saved.")
    else:
